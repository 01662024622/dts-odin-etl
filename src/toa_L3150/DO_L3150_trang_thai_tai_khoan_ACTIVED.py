...
def main():
    ...
    # try:
    #     df_flag = spark.read.parquet("s3://dtsodin/flag/flag_trang_thai_tai_khoan_actived.parquet")
    ...
    if dyf_tpe_enduser_used_product_history.count() > 0:
        try:
            ...
            ######################################
            ######## START actived
            dyf_join_actived_status = Filter.apply(frame=dyf_join_part_one_contact,
                                                   f=lambda x: x["status_new"] == ACTIVED
                                                               or x["status_new"] == SUSPENDED
                                                               or x["status_new"] == EXPIRED
                                                               or x["status_new"] == CANCELLED)
            print "dyf_join_actived_status ", dyf_join_actived_status.count()
            dyf_join_actived_status.toDF().show(2)
            df_join_actived_status = dyf_join_actived_status.toDF()

            df_join_actived_status = df_join_actived_status \
                .withColumn("change_status_date_id",
                            from_unixtime(df_join_actived_status.timecreated, 'yyyyMMdd').cast("long")) \
                .withColumn("from_status_id", f.lit(None).cast("long")) \
                .withColumn("to_status_id", f.lit(205).cast("long")) \
                .withColumn("measure1", f.lit(None).cast("long")) \
                .withColumn("measure2", f.lit(None).cast("long")) \
                .withColumn("description", df_join_actived_status.status_description) \
                .withColumn("timestamp1", f.lit(None).cast("long"))
            df_join_actived_status.show(3)
            dyf_join_actived_status = DynamicFrame.fromDF(df_join_actived_status, glueContext,
                                                          "dyf_join_actived_status")

            dyf_join_actived_status = dyf_join_actived_status \
            ...
            dyf_join_actived_status.printSchema()
            df_join_actived_status = dyf_join_actived_status.toDF()
            ####### END

            df_join_actived_status = df_join_actived_status.withColumn("user_id", f.lit(None).cast("long"))

            dyf_join_status = DynamicFrame.fromDF(df_join_actived_status, glueContext, "dyf_join_status")
            ...
            # ghi de _key vao s3
            df.write.parquet("s3a://dtsodin/flag/flag_trang_thai_tai_khoan_actived.parquet", mode="overwrite")
