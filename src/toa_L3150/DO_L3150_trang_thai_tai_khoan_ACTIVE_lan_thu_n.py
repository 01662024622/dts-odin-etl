...
def main():
    ...
    # try:
    #     df_flag = spark.read.parquet("s3://dtsodin/flag/flag_trang_thai_tai_khoan_active_lan_n.parquet")
    ...
    if dyf_tpe_enduser_used_product_history.count() > 0:
        try:
            ...
            dyf_join_active_status = Filter.apply(frame=dyf_join_part_one_contact,
                                                     f=lambda x: x["status_new"] == ACTIVE)
            ...
            df_join_active_status = df_join_active_status \
                ...
                .withColumn("to_status_id", f.lit(212).cast("long")) \
            ...
            # ghi de _key vao s3
            df.write.parquet("s3a://dtsodin/flag/flag_trang_thai_tai_khoan_active_lan_n.parquet", mode="overwrite")
