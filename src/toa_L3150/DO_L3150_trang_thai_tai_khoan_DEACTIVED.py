...
def main():
    ...
    # try:
    #     df_flag = spark.read.parquet("s3://dtsodin/flag/flag_trang_thai_tai_khoan_deactived.parquet")
    ...
    if dyf_tpe_enduser_used_product_history.count() > 0:
        try:
            ...
            df_tpe_used_product_history_step1 = df_tpe_enduser_used_product_history.groupby('contact_id', 'used_product_id').agg(
            ...
            dyf_tpe_used_product_history = DynamicFrame.fromDF(df_tpe_used_product_history_step2, glueContext, "dyf_tpe_used_product_history")
            ...
            ######################################
            ######## START DEACTIVED
            dyf_join_deactived_status = Filter.apply(frame=dyf_join_part_one_contact,
                                                     f=lambda x: x["status_new"] == DEACTIVED)
            print "dyf_join_deactived_status ", dyf_join_deactived_status.count()
            dyf_join_deactived_status.toDF().show(2)
            df_join_deactived_status = dyf_join_deactived_status.toDF()

            df_join_deactived_status = df_join_deactived_status \
                .withColumn("change_status_date_id",
                            from_unixtime(df_join_deactived_status.timecreated, 'yyyyMMdd').cast("long")) \
                .withColumn("from_status_id", f.lit(None).cast("long")) \
                .withColumn("to_status_id", f.lit(204).cast("long")) \
                .withColumn("measure1", f.lit(None).cast("long")) \
                .withColumn("measure2", f.lit(None).cast("long")) \
                .withColumn("description", df_join_deactived_status.status_description) \
                .withColumn("timestamp1", f.lit(None).cast("long"))
            df_join_deactived_status.show(3)
            dyf_join_deactived_status = DynamicFrame.fromDF(df_join_deactived_status, glueContext,
                                                            "dyf_join_deactived_status")

            dyf_join_deactived_status = dyf_join_deactived_status \
            ...
            dyf_join_deactived_status.printSchema()
            df_join_deactived_status = dyf_join_deactived_status.toDF()
            ####### END

            df_join_deactived_status = df_join_deactived_status.withColumn("user_id", f.lit(None).cast("long"))

            dyf_join_status = DynamicFrame.fromDF(df_join_deactived_status, glueContext, "dyf_join_status")
            ...
            # ghi de _key vao s3
            df.write.parquet("s3a://dtsodin/flag/flag_trang_thai_tai_khoan_deactived.parquet", mode="overwrite")
