...
def main():
    ...
    # try:
    #     df_flag = spark.read.parquet("s3://dtsodin/flag/flag_trang_thai_tai_khoan_expired.parquet")
    ...
    if dyf_tpe_enduser_used_product_history.count() > 0:
        try:
            ...
            df_join_expired_status = df_join_expired_status \
                ...
                .withColumn("to_status_id", f.lit(209).cast("long")) \
            ...
            # ghi de _key vao s3
            df.write.parquet("s3a://dtsodin/flag/flag_trang_thai_tai_khoan_expired.parquet", mode="overwrite")
