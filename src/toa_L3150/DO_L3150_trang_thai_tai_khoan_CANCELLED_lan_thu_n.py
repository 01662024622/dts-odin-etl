...
def main():
    ...
    # try:
    #     df_flag = spark.read.parquet("s3://dtsodin/flag/flag_trang_thai_tai_khoan_cancelled_lan_n.parquet")
    ...
    if dyf_tpe_enduser_used_product_history.count() > 0:
        try:
            ...
            df_join_cancelled_status = df_join_cancelled_status.withColumn("user_id", f.lit(None).cast("long"))

            dyf_join_status = DynamicFrame.fromDF(df_join_cancelled_status, glueContext, "dyf_join_status")
            ...
            # ghi de _key vao s3
            df.write.parquet("s3a://dtsodin/flag/flag_trang_thai_tai_khoan_cancelled_lan_n.parquet", mode="overwrite")
