...
def main():
    ...
    dyf_student_contact = dyf_student_contact.select_fields(
        ['contact_id', 'student_id', 'time_lms_created']).rename_field('contact_id', 'contact_id_main')
    dyf_student_contact = dyf_student_contact.resolveChoice(specs=[('time_lms_created', 'cast:long')])


    # time_lms_created_from = 01/01/2019
    time_lms_created_from = 1546304400
    # time_lms_created_to = 01/10/2019
    time_lms_created_to = 1569891600
    ...
    dyf_student_contact = Filter.apply(frame=dyf_student_contact, f=lambda x: x['student_id'] is not None
                                                                              and x['contact_id_main'] is not None
                                                                              and x['time_lms_created'] is not None
                                                                              and x[
                                                                                  'time_lms_created'] <= time_lms_created_to)

    print('df_student_contact count 1:', dyf_student_contact.count())
    dyf_student_contact = dyf_student_contact.select_fields(
        ['contact_id_main', 'student_id'])
    ...
    if dyf_student_contact.count() > 0:
        try:
            ...
            ########## dyf_learning_object_class
            dyf_student_daily_info = glueContext.create_dynamic_frame.from_catalog(
                database="tig_advisor",
                table_name="student_daily_info"
            )
            dyf_student_daily_info = Filter.apply(frame=dyf_student_daily_info, f=lambda x: x['_key'] <= time_lms_created_to)

            dyf_student_daily_info = dyf_student_daily_info.select_fields(['_key', 'contact_id', 'product_status',
                                                                           'level_study', 'time_created'])
            df_student_daily_info = dyf_student_daily_info.toDF()
            df_student_daily_info_max = df_student_daily_info.groupby('contact_id').agg(f.max('time_created').alias('max_key')) \
                .withColumnRenamed("contact_id", "contact_id_max")
            print('SHOW df_student_daily_info_max: ', df_student_daily_info_max.count())

            df_student_daily_info_join = df_student_daily_info_max.join(df_student_daily_info,
                                                               (df_student_daily_info['contact_id']
                                                                == df_student_daily_info_max['contact_id_max']) & (
                                                                       df_student_daily_info['time_created']
                                                                       == df_student_daily_info_max['max_key']))

            df_student_daily_info_join.printSchema()
            df_student_daily_info_join.show(5)

            dyf_student_daily_info = DynamicFrame.fromDF(df_student_daily_info_join, glueContext, "dyf_student_daily_info")
            dyf_student_daily_info = Filter.apply(frame=dyf_student_daily_info, f=lambda x: (x['product_status'] == 'ACTIVED'
                                                       or x['product_status'] == 'SUSPENDED'))

            # dyf_student_daily_info = dyf_student_daily_info.select_fields(['contact_id_max' 'product_status', 'level_study'])

            # dyf_student_contact.printSchema()
            print('SHOW 1: ', dyf_student_daily_info.count())
            dyf_student_daily_info.printSchema()
            dyf_student_daily_info.show()

            dyf_student_contact = Join.apply(dyf_student_contact, dyf_student_daily_info, 'contact_id_main', 'contact_id_max')

            print('SHOW 11: ', dyf_student_contact.count())
            dyf_student_contact.printSchema()
            dyf_student_contact.show(5)
            ...
            df_student_contact = df_student_contact.withColumn('modified_date_id', f.lit(date_id)) \
                .withColumn('modified_time', f.lit(modified_time)) \
                .withColumn('created_date_id', f.lit(date_id))
            ...
            df_student_contact.show(5)
            print('SHOW 1111: ', df_student_contact.count())
            ...
            df_mapping_lo_class = df_mapping_lo_class.withColumn("knowledge_1",
                                                                 df_mapping_lo_class.knowledge * f.lit(100)) \
                .withColumn("comprehension_1", df_mapping_lo_class.comprehension * f.lit(100)) \
                .withColumn("application_1", df_mapping_lo_class.application * f.lit(100)) \
                .withColumn("analysis_1", df_mapping_lo_class.analysis * f.lit(100)) \
                .withColumn("synthesis_1", df_mapping_lo_class.synthesis * f.lit(100)) \
                .withColumn("evaluation_1", df_mapping_lo_class.evaluation * f.lit(100)) \
                .withColumn("knowledge_pass_date_id", add_pass_date(df_mapping_lo_class.knowledge, f.lit(date_id))) \
                .withColumn("comprehension_pass_date_id",
                            add_pass_date(df_mapping_lo_class.comprehension, f.lit(date_id))) \
                .withColumn("application_pass_date_id", add_pass_date(df_mapping_lo_class.application, f.lit(date_id))) \
                .withColumn("analysis_pass_date_id", add_pass_date(df_mapping_lo_class.analysis, f.lit(date_id))) \
                .withColumn("synthesis_pass_date_id", add_pass_date(df_mapping_lo_class.synthesis, f.lit(date_id))) \
                .withColumn("evaluation_pass_date_id", add_pass_date(df_mapping_lo_class.evaluation, f.lit(date_id)))
            ...
            applymapping = ApplyMapping.apply(frame=dyf_student_lo_init,
                                              mappings=[("student_id", "string", "student_id", "long"),
                                                        ...
                                                        ("max_knowledge_pass_date_id", 'string', 'knowledge_pass_date_id',
                                                         'long'),
                                                        ...
                                                        ("max_analysis_pass_date_id", 'string', 'analysis_pass_date_id',
                                                         'long'),
                                                        ("max_synthesis_pass_date_id", 'string', 'synthesis_pass_date_id',
                                                         'long'),
                                                        ("max_evaluation_pass_date_id", 'string', 'evaluation_pass_date_id',
                                                         'long'),
            ...
            datasink5 = glueContext.write_dynamic_frame.from_jdbc_conf(frame=dyf_student_lo_init,
                                                                       ...
                                                                       connection_options={
                                                                           "dbtable": "mapping_lo_student_init",
                                                                           "database": "dts_odin"
                                                                       },
