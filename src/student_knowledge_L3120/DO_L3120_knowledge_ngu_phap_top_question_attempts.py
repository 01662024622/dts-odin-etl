...
def main():
    ...
    knowledge = ['85', '86', '87', '88', '89', '90', '91', '92']
    # Hieu
    comprehension = ['87', '88', '89', '90', '92']
    # Ap dung
    application = ['90', '92']
    # Phan tich
    analysis = ['92']
    ...
    # try:
    #     # # doc moc flag tu s3
    #     df_flag = spark.read.parquet("s3a://dtsodin/flag/flag_question_attempts_steps_grammar.parquet")
    ...
    if dyf_top_question_attempts.count() > 0:
        ...
        dyf_top_question_attempt_steps = dyf_top_question_attempt_steps.select_fields(
            ['_key', 'id', 'questionattemptid', 'state']).rename_field('id', 'steps_id')
        ...
        dyf_mapping_lo_lc = glueContext.create_dynamic_frame.from_catalog(
            database="moodle",
            table_name="mapping_grammar_lo"
        )
        dyf_mapping_lo_lc = dyf_mapping_lo_lc.select_fields(
            ['question', 'lo', 'lc'])

        df_mapping_lo_lc = dyf_mapping_lo_lc.toDF()
        df_mapping_lo_lc = df_mapping_lo_lc.drop_duplicates()
        dyf_mapping_lo_lc = DynamicFrame.fromDF(df_mapping_lo_lc, glueContext, "dyf_mapping_lo_lc")
        dyf_mapping_lo_lc.printSchema()
        ...
        dyf_join03.show(2)
        dyf_mapping_lo_lc.printSchema()
        dyf_mapping_lo_lc.show(2)
        # df_mapping_lo_lc = dyf_mapping_lo_lc.toDF()
        # df_mapping_lo_lc = df_mapping_lo_lc.withColumn("question_id", f.lit(52059))
        # dyf_mapping_lo_lc = DynamicFrame.fromDF(df_mapping_lo_lc, glueContext, "dyf_mapping_lo_lc")
        # dyf_mapping_lo_lc.printSchema()
        # dyf_mapping_lo_lc.show(2)
        print "total: ", dyf_join03.count()
        dyf_join03 = Join.apply(dyf_join03, dyf_mapping_lo_lc, 'quest_id', 'question')
        dyf_join03.printSchema()
        print("@@@@@@@@@@@@@@@@@@@@@")
        dyf_join03.printSchema()
        #
        ...
        dyf_join03 = dyf_join03.select_fields(
            ['student_id', 'rightanswer', 'timemodified', 'state', 'name', 'parent', 'questionid', 'lo', 'lc'])
        dyf_join03.printSchema()
        dyf_join03.show()
        print('COUNT:::', dyf_join03.count())
        ...
        dyf_right = dyf_join03
        # df_right.cache()
        if (dyf_right.count() > 0):
            try:
            #     df_right = df_right.withColumn("lc", add_code())\
            #                        .withColumn("lo", f.lit('G5'))
                # df_right = df_right.withColumn("lo", f.lit('G5'))
                ...
                dyf_learning_object = dyf_learning_object.select_fields(
                    ['learning_object_id', 'learning_object_code'])
                # dyf_learning_object.printSchema()
                # dyf_learning_object.show()

                dyf_knowledge_all = Join.apply(dyf_right, dyf_learning_object, 'lo', 'learning_object_code')

                dyf_knowledge_all = dyf_knowledge_all.select_fields(
                    ['student_id', 'learning_object_id', 'name', 'parent', 'timemodified', 'state', 'lc'])
                print ("MMMMMMMMMMMMMMMMM")
                dyf_knowledge_all.show(2)
                # print("COUNT 3:", dyf_knowledge_right.count())
                # dyf_knowledge_all.printSchema()
                # dyf_knowledge_all.show()
                ...
                df_knowledge_all = df_knowledge_all.withColumn("knowledge", addScore(df_knowledge_all['lc'], df_knowledge_all['state'],f.lit('knowledge')))\
                    .withColumn("comprehension", addScore(df_knowledge_all['lc'], df_knowledge_all['state'],f.lit('comprehension')))\
                    .withColumn("application", addScore(df_knowledge_all['lc'], df_knowledge_all['state'], f.lit('application')))\
                    .withColumn("analysis", addScore(df_knowledge_all['lc'], df_knowledge_all['state'], f.lit('analysis')))\
                    .withColumn("synthesis", addScore(df_knowledge_all['lc'], df_knowledge_all['state'], f.lit('synthesis')))\
                    .withColumn("evaluation", addScore(df_knowledge_all['lc'], df_knowledge_all['state'], f.lit('evaluation')))\
                    .withColumn("date_id", from_unixtime(df_knowledge_all['timemodified'], 'yyyyMMdd'))
                print("QQQQQQQQQQQQQQ")
                df_knowledge_all.show(2)
                ...
                df_knowledge_right = df_knowledge_right.groupby('student_id', 'date_id', 'learning_object_id').agg(
                ...
                df_knowledge = df_knowledge.withColumn("user_id",  check_data_null(df_knowledge.student_id_plus, df_knowledge.student_id_minus)) \
                    .withColumn("learning_object_id", check_data_null(df_knowledge.learning_object_id_plus, df_knowledge.learning_object_id_minus)) \
                    .withColumn("created_date_id", check_data_null(df_knowledge.date_id_plus, df_knowledge.date_id_minus)) \
                    .withColumn("source_system", f.lit('top_question_attempt_grammar')) \
                    .withColumn("lu_id", f.lit(0))

                df_knowledge = df_knowledge.drop_duplicates()

                dyf_knowledge_all = DynamicFrame.fromDF(df_knowledge, glueContext, "dyf_knowledge_all")
                print(dyf_knowledge_all.count())
                dyf_knowledge_all.printSchema()
                dyf_knowledge_all.show(2)
            ...
            # ghi de _key vao s3
            df.write.parquet("s3a://dtsodin/flag/flag_question_attempts_steps_grammar.parquet", mode="overwrite")
