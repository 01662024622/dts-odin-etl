...
def main():
    ...
    try:
        # # doc moc flag tu s3
        df_flag = spark.read.parquet("s3a://dtsodin/flag/student_behavior/flag_hoc_vien_duoc_mua_goi_nap_tien.parquet")
    ...
    if (dyf_tpe_invoice_product.count() > 0):
        ...
        dyf_join1 = Join.apply(dyf_student_contact, dyf_join0, "contact_id", "user_id")
        dyf_join = Join.apply(dyf_join1, dyf_log_student_level_study, "user_id", "contact_id")
        print("@@@@@@@@@@@@")
        ...
        # ghi de _key vao s3
        df.write.parquet("s3a://dtsodin/flag/student_behavior/flag_hoc_vien_duoc_mua_goi_nap_tien.parquet", mode="overwrite")
