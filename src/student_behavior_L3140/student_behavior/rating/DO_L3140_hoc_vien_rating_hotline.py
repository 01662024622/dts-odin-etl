...
def main():
    ...
    dyf_advisorcall = dyf_advisorcall.select_fields(
            ['_key', 'calldate', 'phonenumber', 'rating', 'device', 'hanguptvts', 'status']
    )
    dyf_advisorcall = dyf_advisorcall.resolveChoice(specs=[('_key', 'cast:long')])
    try:
    ...
    if dyf_advisorcall.count()> 0:
        ...
        dyf_rating_class = Filter.apply(frame = dyf_advisorcall,
                                            f = lambda x: x['rating'] in ratings
                                                          and x['device'] == '3CX'
                                                          and x['hanguptvts'] == 1
                                                          and x['status'] == 'ANSWER')
        ...
        try:
            ...
            df_join.show(10)
            print "########## . ###########"
            df_join0 = df_rating_class.join(df_student_contact_phone,
                                                      (df_rating_class['phonenumber'] == df_student_contact_phone['phone']))

            df_join01 = df_join0.join(df_student_contact,
                                         (df_join0['contact_id_phone'] == df_student_contact['contact_id']))
            ...
            dyf_join = dyf_join.select_fields(['timecreated', 'contact_id', 'student_id', 'level_study', 'time_lms_created',
                                               'level_current', 'level_modified', 'package_code', 'time_created', 'rating',
                                               'level_modified_new'])
            ...
            dyf_join = dyf_join.select_fields(
                ['timecreated', 'student_id', 'contact_id', 'package_code', 'rating',
            ...
            # ghi de _key vao s3
            df.write.parquet("s3a://dtsodin/flag/flag_hoc_vien_rating_hotline.parquet", mode="overwrite")
