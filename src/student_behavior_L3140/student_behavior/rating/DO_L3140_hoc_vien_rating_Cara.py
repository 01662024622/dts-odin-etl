...
def main():
    ...
    dyf_mdl_rating_class = glueContext.create_dynamic_frame.from_catalog(
        database="topicalms",
        table_name="mdl_rating_class"
    )

    dyf_mdl_rating_class = dyf_mdl_rating_class.select_fields(
            ['_key', 'points', 'vote', 'teacher_id', 'student_id',
             'room_id', 'opinion', 'timecreated', 'timemodified']
    )
    dyf_mdl_rating_class = dyf_mdl_rating_class.resolveChoice(specs=[('_key', 'cast:long')])
    try:
        df_flag_1 = spark.read.parquet("s3://dtsodin/flag/flag_hoc_vien_rating_cara.parquet")
        max_key = df_flag_1.collect()[0]['flag']
        print("max_key:  ", max_key)
        # Chi lay nhung ban ghi lon hon max_key da luu, ko load full
        dyf_mdl_rating_class = Filter.apply(frame=dyf_mdl_rating_class, f=lambda x: x["_key"] > max_key)
    except:
        print('read flag file error ')

    if dyf_mdl_rating_class.count()> 0:
        ...
        dyf_mdl_rating_class.printSchema()
        print dyf_mdl_rating_class.count()
        dyf_rating_class = Filter.apply(frame = dyf_mdl_rating_class,
                                            f = lambda x: x['points'] in points)
        print dyf_rating_class.count()
        try:
            df_rating_class = dyf_rating_class.toDF()
            # df_rating_class = df_rating_class.limit(99999)
            dyf_rating_class_temp = DynamicFrame.fromDF(df_rating_class, glueContext, "dyf_rating_class_temp")
            ...
            df_join01 = df_rating_class.join(df_student_contact,
                                         (df_rating_class['student_id'] == df_student_contact['student_id_contact']))
            ...
            dyf_join = dyf_join.select_fields(['timecreated', 'contact_id', 'student_id', 'level_study', 'time_lms_created',
                                               'level_current', 'level_modified', 'package_code', 'time_created', 'opinion', 'vote',
                                               'points', 'level_modified_new'])
            ...
            dyf_join.show(10)
            dyf_join = dyf_join.select_fields(
                ['timecreated', 'student_id', 'contact_id', 'package_code', 'opinion', 'vote' , 'points',
            ...
            df_join04 = df_join04.withColumn("transformed_at", unix_timestamp(f.current_timestamp())) \
                ...
                .withColumn("comment", df_join04.opinion) \
                .withColumn("rating_about", df_join04.vote) \
                .withColumn("number_rating", f.lit(1)) \
                .withColumn("value_rating", df_join04.points)
            ...
            applymapping0 = ApplyMapping.apply(frame=dyf_rating_cara,
                                               mappings=[
                                                   ...
                                                   ("rating_type", "string", "rating_type", "string"),
                                                   ("comment", "string", "comment", "string"),
                                                   ("rating_about", "long", "rating_about", "string"),
            ...
            df_temp = dyf_rating_class_temp.toDF()
            flag = df_temp.agg({"_key": "max"}).collect()[0][0]

            flag_data = [flag]
            df = spark.createDataFrame(flag_data, "long").toDF('flag')
            # ghi de _key vao s3
            df.write.parquet("s3a://dtsodin/flag/flag_hoc_vien_rating_cara.parquet", mode="overwrite")
