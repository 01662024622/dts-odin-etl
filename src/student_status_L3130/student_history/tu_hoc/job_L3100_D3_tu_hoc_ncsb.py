...
def main():
    ...
    dyf_ds_results = glueContext.create_dynamic_frame.from_catalog(database='ncsbasic', table_name='results')
    ...
    if (dyf_ds_results.count() > 0):
        ...
        # -------------------------------------------------------------------------------------------------------------#
        dyf_users = glueContext.create_dynamic_frame.from_catalog(database='ncsbasic', table_name='users')
        ...
        datasink2 = glueContext.write_dynamic_frame.from_jdbc_conf(frame=dropnullfields2,
                                                                   ...
                                                                   connection_options={"dbtable": "temp_staging_lich_su_tu_hoc_ncsb",
                                                                                       "database": "dts_odin",
                                                                                       "postactions": """INSERT into mapping_changed_status_student(user_id, change_status_date_id, to_status_id, measure1, measure2)
                                                                                                            ...
                                                                                                            FROM temp_staging_lich_su_tu_hoc_ncsb hwb
                                                                                                            LEFT JOIN user_map um
                                                                                                                 ON um.source_type = 1
                                                                                                                 AND um.source_id = hwb.contact_id
                                                                                                            WHERE um.user_id is not null;
                                                                                                             DROP TABLE IF EXISTS public.temp_staging_lich_su_tu_hoc_ncsb"""
                                                                                       },
                                                                   redshift_tmp_dir="s3n://dts-odin/temp/tu-hoc/ncsb",
