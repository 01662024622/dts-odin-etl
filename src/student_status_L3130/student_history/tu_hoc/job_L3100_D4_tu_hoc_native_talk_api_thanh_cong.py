...
def main():
    ...
    dyf_native_talk = dyf_native_talk.resolveChoice(specs=[('_key', 'cast:long')])

    try:
        df_flag = spark.read.parquet("s3a://dts-odin/flag/student_status/tu_hoc/tu_hoc_native_talk_thanh_cong.parquet")
    ...
    dyf_native_talk = dyf_native_talk.select_fields(
        ['_key', 'learning_date', 'speaking_dialog_score', 'username', 'updated_time'])
    ...
    if (dyf_native_talk.count() > 0):
        ...
        dyf_native_talk = Filter.apply(frame=dyf_native_talk,
                                              f=lambda x: x["username"] is not None and x["username"] != ''
                                                          and x["speaking_dialog_score"] is not None and x["speaking_dialog_score"] > 0
        ...
        if (dyf_native_talk.count() > 0):
            ...
            if(join.count() > 0):
                ...
                datasink2 = glueContext.write_dynamic_frame.from_jdbc_conf(frame=dropnullfields2,
                                                                           ...
                                                                           connection_options={"dbtable": "temp_staging_lich_su_tu_hoc_native_talk___",
                                                                                               "database": "dts_odin",
                ...
                df.write.parquet("s3a://dts-odin/flag/student_status/tu_hoc/tu_hoc_native_talk_thanh_cong.parquet", mode="overwrite")
                dy_cache.unpersist()
