...
# try:
#     # # doc moc flag tu s3
#     df_flag = spark.read.parquet("s3a://dts-odin/flag/fact_flag_LiveStream.parquet")
...
if (fact_data_livestream.count() > 0):
    # Loc du lieu => loai bo cac du lieu null
    ...
    fact_data_livestream = DynamicFrame.fromDF(fact_ds_livestream, glueContext, "join_livestream")
    ...
    # co hoc LT thanh cong
    if (data_livestream_tc.count() > 0):

        # loc dl theo cong thuc measure2 >25 phut , tinh so luot LT thanh cong
        ds_livestream = data_livestream_tc.toDF()
        ds_livestream = ds_livestream.groupby('student_id','date_id').agg(f.countDistinct("room_id").alias("measure1"), f.sum("time_study").alias("measure2"))
        data_livestream_tc = DynamicFrame.fromDF(ds_livestream, glueContext, "data_livestream")
        # measure1 so luot , measure2 so phut hoc
        ...
        # co hoc LT thanh cong measure2 > 25 phut <=> measure2 > 25*60 ( measure2 tinh bang giay )
        data_livestream_tc = Filter.apply(frame=data_livestream_tc, f=lambda x: x['measure2'] > 25 * 60)
