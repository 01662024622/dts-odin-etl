...
def main():
    ...
    try:
        # # doc moc flag tu s3
        df_flag = spark.read.parquet("s3a://dts-odin/flag/fact_flag_canceled.parquet")
    ...
    if (mdl_tpe_enduser_used_product_history.count() > 0):
        ...
        df_mdl_tpe_enduser_used_product_history = df_mdl_tpe_enduser_used_product_history.withColumn(
            'change_status_date_id', from_unixtime(
                df_mdl_tpe_enduser_used_product_history['timecreated'], "yyyyMMdd"))\
            .withColumn('to_status_id', f.lit(106))\
            .withColumn('timestamp1', df_mdl_tpe_enduser_used_product_history['timecreated'] * f.lit(1000))
                                                                   ...
                                                                   connection_options={
                                                                       ...
                                                                       "database": "dts_odin",
                                                                        "postactions":"""UPDATE mapping_changed_status_student
	                                                                                     ...
	                                                                                     WHERE user_id IS NULL and to_status_id=106"""
                                                                   },
        ...
        # ghi de _key vao s3
        df.write.parquet("s3a://dts-odin/flag/fact_flag_canceled.parquet", mode="overwrite")
